...
from utils.label_utils import normalize_label, explanation_contains_label_stem
    ...
    def classify(self, text: str, labels: List[str], normalized_labels: Dict[str, str]) -> Dict:
        ...
        try:
            ...
            for k in score_block:
                print(f"Normalized '{k}' → '{normalize_label(k)}'")
            ...
            for norm_label, canonical_label in normalized_labels.items():
                if explanation_contains_label_stem(explanation, norm_label) and parsed_scores.get(canonical_label, 0.0) < 0.1:
